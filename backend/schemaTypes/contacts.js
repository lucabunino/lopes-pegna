...
export default {
	...
	fields: [
		...
		},
		{
			...
			options: {columns: 2},
			fields: [
				{name: 'instagramHandle', title: 'Handle', type: 'string'},
				{name: 'instagramHref', title: 'Href', type: 'url'},
			],
		},
		{
		...
		},
		{
			...
			of: [
				{name: 'image', type: 'image', icon: ImageIcon},
				{
					...
					preview: {
						select: {filename: 'video.asset.originalFilename', poster: 'poster'},
						prepare(selection) {
							const {filename, poster} = selection
							return {title: filename, media: poster || DocumentVideoIcon}
						},
					},
				},
			],
